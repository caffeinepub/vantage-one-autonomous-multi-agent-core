...
export default function AffiliatePerformance() {
  ...
  const totalClicks =
    performanceSummary?.clickCounts.reduce(
      (sum, [_, clicks]) => sum + Number(clicks),
      0,
    ) || 0;
  const avgConversionRate = performanceSummary?.conversionRates.length
    ? performanceSummary.conversionRates.reduce(
        (sum, [_, rate]) => sum + rate,
        0,
            ...
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-chart-2">
                  {topPerformer.clicks.toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Conversion Rate</p>
                <p className="text-lg font-bold text-chart-3">
                  {(topPerformer.conversionRate * 100).toFixed(1)}%
                </p>
              </div>
              <div>
        ...
        <CardContent>
          {offerPerformanceData.length > 0 ? (
            <ScrollArea className="h-96">
              <div className="space-y-3">
                {offerPerformanceData.map((data, index) => {
                  ...
                  const trendIcon =
                    data.conversionRate > avgConversionRate
                      ? ArrowUpRight
                      : ArrowDownRight;
                  ...
                  return (
                    <Card
                      key={data.offer.id}
                      className={`p-4 ${isTopPerformer ? "border-chart-2 bg-chart-2/5" : ""}`}
                    >
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-4 text-xs text-muted-foreground">
                              <span>
                                Price: $
                                {(
                                  Number(data.offer.priceInCents) / 100
                                ).toFixed(2)}
                              </span>
                              <span>
                                Commission:{" "}
                                {(data.offer.commissionRate * 100).toFixed(0)}%
                              </span>
                            </div>
                          </div>
                        ...
                        {data.lastUpdated && (
                          <p className="text-xs text-muted-foreground">
                            Last updated:{" "}
                            {new Date(
                              Number(data.lastUpdated) / 1000000,
                            ).toLocaleString()}
                          </p>
                        )}
                      </div>
            ...
            <div className="flex h-64 items-center justify-center text-sm text-muted-foreground">
              No performance data yet. Agents will track metrics as offers are
              promoted.
            </div>
          )}
        </CardContent>
